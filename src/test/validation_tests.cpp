...
static void TestBlockSubsidyHalvings(const Consensus::Params& consensusParams)
{
    ...
    for (int nHalvings = 0; nHalvings < maxHalvings; nHalvings++) {
        ...
        CAmount nSubsidy = GetBlockSubsidy(nHeight, false, 0, 0, consensusParams);
        BOOST_CHECK(nSubsidy <= nInitialSubsidy);
        BOOST_CHECK_EQUAL(nSubsidy, nPreviousSubsidy / 2);
        nPreviousSubsidy = nSubsidy;
    }
    BOOST_CHECK_EQUAL(GetBlockSubsidy(maxHalvings * consensusParams.nSubsidyHalvingInterval, false, 0, 0, consensusParams), 0);
}
...
BOOST_AUTO_TEST_CASE(subsidy_limit_test)
{
    ...
    for (int nHeight = 0; nHeight < 14000000; nHeight += 1000) {
        CAmount nSubsidy = GetBlockSubsidy(nHeight, false, 0, 0, chainParams->GetConsensus());

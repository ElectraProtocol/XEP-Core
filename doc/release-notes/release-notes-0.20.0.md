...
XEP Core version 0.20.0 is now available from:

  <https://bitcoincore.org/bin/xep-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from XEP Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since XEP Core version 0.18.0.
...
- Testing or debugging of implementations of the XEP P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of XEP Core. XEP Core logs debug messages
...
- OpenSSL is no longer used by XEP Core.  (#17265)

- BIP70 support has been fully removed from XEP Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  `xepd help` for more information.  This option is experimental and
...
- All custom settings configured when XEP Core starts are now
...
- The "Start XEP Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://bitcoincore.org/en/releases/0.18.0/#wallet-gui)
  ...
  affected anyone who simultaneously used multiple XEP Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed XEP Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/xep-core/HWI) for
  signing.  Future versions of XEP Core should support a GUI option
...
- XEP Core's automatically-generated source code documentation is
...
- The `xep-cli` utility used with the `-getinfo` parameter now
...
- The `xep-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `xepstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_xep-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_xep` binary (hebasto)
...
- #18702 Fix ASLR for xep-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate xep schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest xep.conf (sdaftuar)
...
- #18022 Fix appveyor `test_xep` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (xep-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone xep-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about XEP Core files (hebasto)
...
- #17119 Fix broken xep-cli examples (andrewtoth)
...
- #17503 Remove bitness from xep-qt help message and manpage (laanwj)
...
- #17648 Rename wallet-tool references to xep-wallet (hel-o)
...
- #18341 Replace remaining literal XEP with `CURRENCY_UNIT` (domob1812)
...
- #16115 On xepd startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from xep-cli -getinfo (malevolent)

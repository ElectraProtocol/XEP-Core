...
export P_CI_DIR="${BASE_BUILD_DIR}/xep-$HOST"
...
if [[ ${USE_MEMORY_SANITIZER} == "true" ]]; then
  ...
  # details.
  DOCKER_EXEC 'grep -v HAVE_SYS_GETRANDOM src/config/xep-config.h > src/config/xep-config.h.tmp && mv src/config/xep-config.h.tmp src/config/xep-config.h'
fi
